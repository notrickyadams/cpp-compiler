\section{Threats to Validity --- draft v1}
\label{sec:threats}

\textbf{Construct validity.} The central operationalization is stated in
...
loudly against assuming it \cite{denny2014enhancing,pettit2017enhanced}. Likewise "explanation" is
...
\textbf{Internal validity.} The ablation compares four \emph{binaries}, so
...
size, \S9.2). Temporal drift dominated one whole campaign before the
...
process-management quirks of \S8.4 and the measurement-channel
contamination of the discarded campaign (\S9.2) --- each found by
coherence checking; we cannot rule out another we did not trigger.

\textbf{External validity.} One tiny language, one implementation, one
...
artifact is either explicitly argued in \S10 or not made.

\textbf{Conclusion validity.} 30 runs per timing cell; medians with MAD;
